...
// EDIT THESE
int resolution = 50; // How many collums and rows
int scale = 1600; // Higher values = smoother, larger terrain
int peakHeight = 300; // Higher values = more extreme valleys/mountains
  ...
  float yoff = 0;
   for (int y = 0; y < rows+1; y++){
    ...
    for (int x = 0; x < cols; x++){
      terrain[x][y] = map(noise(xoff,yoff), 0, 1, -peakHeight, 0);
      xoff += 0.1;
    }
    yoff += 0.1;
  }
...
void draw() {
  ...
  for (int y = 0; y < rows; y++){
    ...
    for (int x = 0; x < cols; x++){
      ...
      fill(map(terrain[x][y],-peakHeight,0,-20,120),100,100); // Set our heatmap
